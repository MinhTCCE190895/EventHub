...
public class VenueService : IVenueService
{
    ...
    public async Task UpdateVenueAsync(VenueUpdateDTO dto, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException($"Không th? gi?m s?c ch?a xu?ng {dto.MaxCapacity} vì dang có s? ki?n có {maxRegistered} lu?t dang ký.");
        }
    ...
    public async Task DeleteVenueAsync(int id, CancellationToken cancellationToken = default)
    {
        ...
        {
            throw new InvalidOperationException("Không th? xóa d?a di?m này vì dang có s? ki?n du?c t? ch?c t?i dây");
        }
