...
- **2026-06-23 (Antigravity)**:
  - **QuiNC - Bảo vệ file appsettings.json & Khóa local connection strings (`FE-03`)**: Cấu hình quy tắc chặn tự ý sửa đổi file cấu hình và chuỗi kết nối vào file quy chuẩn chung `01-token-and-docs.md`. Đồng thời thực hiện chạy lệnh `git update-index --assume-unchanged` trên cả 3 dự án (`Blazor`, `MVC`, `RazorPages`) để tránh commit đè cấu hình cơ sở dữ liệu cục bộ giữa LocalDB và SQL Server đầy đủ của các thành viên.
