...
- **2026-07-21 (Antigravity / Agent)**:
  - **Sửa lỗi bảo mật IDOR (Insecure Direct Object Reference) và hoàn thiện luồng kiểm soát trạng thái sự kiện**:
    ...
    - Ngăn chặn tạo và sửa sự kiện trong quá khứ: Thêm logic kiểm tra `StartTime < DateTime.Now` vào hàm `Validate` của `EventCreateDTO` (đối với tạo mới). Đối với Update, kiểm tra ở `EventService.cs`: chỉ báo lỗi nếu Organizer cố tình thay đổi `StartTime` thành một mốc thời gian ở quá khứ (khác với `StartTime` cũ trong DB), giúp bảo vệ tính toàn vẹn dữ liệu lịch sử mà vẫn cho phép sửa lỗi chính tả ở các sự kiện đã qua.
