...
- **2026-07-21 (Antigravity / Agent)**:
  - **Sửa lỗi bảo mật IDOR (Insecure Direct Object Reference) và hoàn thiện luồng kiểm soát trạng thái sự kiện**:
    ...
    - Ngăn chặn tạo và sửa sự kiện trong quá khứ: Thêm logic kiểm tra `StartTime < DateTime.Now` vào hàm `Validate` của `EventCreateDTO` và `EventUpdateDTO`. Yêu cầu mọi sự kiện mới (hoặc chỉnh sửa) đều phải có thời gian bắt đầu ở tương lai, đảm bảo tính toàn vẹn dữ liệu lịch sử và logic báo cáo.
